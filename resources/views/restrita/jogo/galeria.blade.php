    ...
    @foreach($galeria as $jogo)
        ...
        <div class="col-xs-6 col-md-4">
            <div class="thumbnail no-bg b-a-2 b-gray-dark">
                <img src="{{ asset('uploads/'.Auth::user()->uuid.'/prints/'. $jogo->print) }}" alt="100%x200" data-holder-rendered="true" data-toggle="modal" data-target="#printModal" data-titulo="{{"{$jogo->titulo} - {$jogo->platinado_em}"}}" data-print="{{ asset('uploads/'.Auth::user()->uuid.'/prints/'. $jogo->print) }}" data-descricao="{{ "Plataforma: {$jogo->plataforma} - Publicado por: {$jogo->publisher} - Dificuldade da platina: {$jogo->dificuldade}"}}">
                <div class="caption">
