...
class DashboardController extends Controller
{
    ...
    public function dashboard(){
        ...
        if($this->user->jogos()->count() > 0){
            ...
            $graficos['platinasPorMes'] = DB::table('jogos')->selectRaw('MONTH(platinado_em) AS mes_numero, (CASE WHEN MONTH(platinado_em)=1 THEN "JAN" WHEN MONTH(platinado_em)=2 THEN "FEV" WHEN MONTH(platinado_em)=3 THEN "MAR" WHEN MONTH(platinado_em)=4 THEN "ABR" WHEN MONTH(platinado_em)=5 THEN "MAI" WHEN MONTH(platinado_em)=6 THEN "JUN" WHEN MONTH(platinado_em)=7 THEN "JUL" WHEN MONTH(platinado_em)=8 THEN "AUG" WHEN MONTH(platinado_em)=9 THEN "SET" WHEN MONTH(platinado_em)=10 THEN "OUT" WHEN MONTH(platinado_em)=11 THEN "NOV" ELSE "DEZ" END) AS mes_texto, COUNT(*) as qtd')->where('user_id', Auth::user()->id)->groupBy('mes_numero', 'mes_texto')->orderBy('mes_numero', 'ASC')->get();
